...
- `call_logs` — per-call metadata, transcript, direction, caller_id, destination
...
- `api_keys` — per-user REST API keys (key_hash, label, last_used_at; FK → users)
...
**API authentication** — `api_auth_middleware` checks `/api/*` in this order:
1. **Session cookie** — browser/OAuth session, scoped to that user
2. **Global `API_KEY` env var** — admin-level, no owner filter
3. **Per-user key** (`sk-va-<64hex>`) — looked up in `api_keys` table by SHA-256 hash, scoped to that user

Per-user keys are managed at **Admin UI → avatar → API Keys**. Each key has a label and `last_used_at` timestamp. Only the SHA-256 hash is stored — raw key shown once at creation. Users can create multiple keys and revoke them individually.

**Per-user data isolation:** agents, flows, phone_routes, and call_logs are filtered by `owner_id`. The helper `auth.get_owner_id(request)` returns the caller's UUID or `None` for admin/dev/global-key (sees all). All queries use the pattern:
...
**Caller ID capture for inbound calls:** the Asterisk dialplan (generated by `docker_manager.write_extensions_conf`) calls `CURL(http://config-api:8080/internal/calls/pre-register, ...)` before `AudioSocket`, passing `CALLERID(num)` and the dialed DID. The `/internal/calls/pre-register` endpoint upserts `call_logs` so caller info appears immediately. Outbound calls store `caller_id` from the originate request at pre-create time.

**Key files:**
- `config-api/auth.py` — `get_session_user()`, `get_owner_id()`, `get_user_for_api_key()`, middlewares
...
- `config-api/routers/api_keys.py` — `GET/POST/DELETE /api/keys`
...
- `config-api/migrations/011_caller_id.sql` — caller_id column on call_logs
- `config-api/migrations/012_api_keys.sql` — api_keys table (user_id, key_hash, label, last_used_at)
