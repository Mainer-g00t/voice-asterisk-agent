...
**`conversation`** 🟦 — STT → LLM → TTS loop. Stays in this node until an outgoing edge condition fires.

| Config field | Type | Description |
|---|---|---|
| `system_prompt` | string | System prompt for the LLM. Supports `{placeholder}` substitution from `template_vars`. |
| `greeting` | string | Optional opening message spoken when this node is entered. |
| `silence_timeout_seconds` | number | Seconds of silence before a `silence_timeout` edge fires (default: none). |

---

**`say`** 🟨 — Plays a fixed TTS message verbatim, then immediately takes the `default` edge. No STT/LLM involved.

| Config field | Type | Description |
|---|---|---|
| `message` | string | Text to speak. Supports `{placeholder}` substitution. |

---

**`gather_dtmf`** 🟧 — Waits for the user to press a keypad digit (0–9, \*, #). Branches via `dtmf_digit` edges. The pressed digit is stored in `last_dtmf` and can also be checked with `variable_equals`.

| Config field | Type | Description |
|---|---|---|
| `prompt` | string | Optional TTS message spoken before waiting for input. |
| `dtmf_timeout` | number | Seconds to wait for a digit (default: 10). |

---

**`transfer`** 🟥 — Issues an Asterisk AMI Redirect. Call is handed off; flow ends.

| Config field | Type | Description |
|---|---|---|
| `destination` | string | Extension or number to transfer to (e.g. `operator`, `+15551234567`). |
| `dialplan_context` | string | Asterisk dialplan context (default: `default`). |

---

**`webhook`** 🟪 — HTTP POSTs the current flow state to a URL. Response is stored in `last_webhook_result`. Branch via `webhook_field` edges or `variable_equals` on `last_webhook_result`.

| Config field | Type | Description |
|---|---|---|
| `url` | string | Endpoint to POST to. Must be reachable from inside the Docker network (e.g. `http://tools-server:8100/check`). |
| `timeout` | number | Request timeout in seconds (default: 10). |

POST body sent to the URL:
```json
{
  "call_uuid": "...",
  "current_node_id": "n2",
  "state": { "turn_count": 3, "last_dtmf": "1", "variables": {} }
}
```

---

**`set_variable`** 🟩 — Writes a value into flow state, then immediately takes the `default` edge. Use before a `condition` node to set up a branch.

| Config field | Type | Description |
|---|---|---|
| `variable_name` | string | Key to write into `state.variables`. |
| `value` | string | Value to store. Supports `{placeholder}` substitution. |

---

**`condition`** 🟫 — Silent branch — evaluates a variable and takes the matching edge. No audio.

| Config field | Type | Description |
|---|---|---|
| `variable_name` | string | Variable to evaluate (any built-in state key or user-defined variable). Draw one `variable_equals` edge per expected value plus a `default` fallback. |

---

**`end`** ⬛ — Hangs up the call and marks the flow execution as `completed`. No config fields.

---

#### Edge condition reference

The **first matching edge** wins. Always add a `default` edge as the last/fallback.

| Condition type | Parameters | Fires when… |
|---|---|---|
| `keyword_matched` | `words: ["bye", "stop"]` | STT transcript of the last user turn contains any of the listed words (case-insensitive). |
| `turn_count_gte` | `n: 5` | Turn counter has reached or exceeded N. |
| `dtmf_digit` | `digit: "1"` | User pressed the specified key (0–9, \*, #). |
| `tool_result` | `tool: "check_crm"` `field: "eligible"` `value: "true"` | A named tool returned a JSON response where the given field equals the given value. |
| `variable_equals` | `var: "last_dtmf"` `value: "1"` | A state variable equals the given value (see state variables below). |
| `silence_timeout` | _(none)_ | No user speech detected for the node's `silence_timeout_seconds`. |
| `webhook_field` | `field: "action"` `value: "transfer"` | The webhook JSON response contains a field equal to the given value. |
| `default` | _(none)_ | Always matches — use as the last edge from any node. |

---

#### Flow state variables

Every flow execution carries a `state` object. All keys are readable via `variable_equals` edges and `condition` nodes.

**Built-in (updated automatically):**

| Variable | Type | Set when |
|---|---|---|
| `turn_count` | string (number) | Incremented after every agent response turn. |
| `last_dtmf` | string | User presses a keypad digit. Value is the single character (`"1"`, `"*"`, etc.). |
| `last_transcript` | string | After every user speech turn — full STT text of what the user said. |
| `last_webhook_result` | string (JSON) | After a `webhook` node completes — full JSON response body as a string. |
| `last_tool_results` | string (JSON) | After a tool is called by the LLM — map of `{tool_name: result}`. |
| `last_intent` | string | If intent detection is used — the classified intent label. |
| `call_status` | string | `"no_answer"` or `"busy"` for outbound calls that weren't answered. |

**User-defined (via `set_variable` nodes):**

Stored under `state.variables`. Readable via `variable_equals` with the variable name you chose.

```json
{"type": "variable_equals", "var": "last_dtmf",     "value": "1"}
{"type": "variable_equals", "var": "turn_count",     "value": "5"}
{"type": "variable_equals", "var": "my_custom_var",  "value": "approved"}
```

User-defined variables take precedence over built-ins on name collision.

---

#### Assigning and running flows

**Assign to an agent:** Admin UI → Agents → edit → 🔀 Flow dropdown → pick a flow → Save. Every call (inbound and outbound) to that agent then runs the flow.
...
The canvas stores flow definitions as JSON in Postgres. `_positions` holds canvas coordinates for the editor and is ignored by the engine.
...
```json
{
  ...
  "_positions": {"n1": {"x": 300, "y": 100}, "n2": {"x": 150, "y": 320}, "n3": {"x": 450, "y": 320}}
}
```
